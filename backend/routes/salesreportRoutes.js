...
// ✅ Get company details from database (no changes)
  ...
  if (byItem === "Month") {
    let monthQuery = `
      SELECT 
        ...
        dgm.DishGroupName,
        cm.CategoryName
      FROM UCSCAFE19.dbo.Vw_MonthwiseSales vw
      LEFT JOIN UCSCAFE19.dbo.DishMaster dm ON vw.DishName = dm.Name
      LEFT JOIN UCSCAFE19.dbo.DishGroupMaster dgm ON dm.DishGroupId = dgm.DishGroupId
      LEFT JOIN UCSCAFE19.dbo.CategoryMaster cm ON dgm.CategoryId = cm.CategoryId
...
// ✅ API for company info (no changes)
  ...
  try {
    ...
    const result = await pool.request().query(`
      SELECT 
        CategoryId,
        CategoryName
      FROM UCSCAFE19.dbo.CategoryMaster 
...
// ✅ API for Dish Groups (no changes)
  ...
  try {
    ...
    let query = `
      SELECT DISTINCT 
        DishGroupId,
        DishGroupName
      FROM UCSCAFE19.dbo.vw_DishWithGroup
...
// ✅ API: Category LOV with all details (no changes)
...
// ✅ API: DishGroup LOV with all details (no changes)
  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        cm.CategoryId,
        cm.CategoryName
      FROM UCSCAFE19.dbo.DishGroupMaster dgm
      LEFT JOIN UCSCAFE19.dbo.CategoryMaster cm ON dgm.CategoryId = cm.CategoryId
...
// ✅ REPLACED: HTML Blob Report (No Puppeteer!)
router.get("/sales-pdf", async (req, res) => {
  try {
    ...
    // ✅ HTML with print button (BLOB method)
    ...
    <head>
      ...
      <style media="print">
        @page { 
          size: ${displayColumns.length > 8 ? 'landscape' : 'portrait'}; 
          margin: 1cm;
        }
        body { 
    ...
    <body>
      ...
      <script>
        // Optional: Auto-open print dialog after 1 second
